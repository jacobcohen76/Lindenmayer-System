...
public class SemiConiferousPythagoreanTree
{
	static int n = 15;
	...
	public static void pythagorean_tree(double A, double B, double width, double height)
	{
		...
		String variables = "X, Y, U, S";
		String axiom = "X";
		String rules = 
				"X = +90 H -90 [ +a > S < ] F [ +90 +90 -b } U { ] -90 H -90 F;\r\n" + 
						"Y = F -90 H -90 [ +a > S < ] F [ +90 +90 -b } U { ] -90 H;\r\n" + 
						"U = +90 H -90 [ +b } X { ] F [ +90 +90 -a > Y < ] -90 H -90 F;\r\n" + 
						"S = F -90 H -90 [ +b } X { ] F [ +90 +90 -a > Y < ] -90 H;";
		...
		try {
			ImageIO.write(image, "png", new File(directory + "Semi-Coniferous Pythagorean Tree - A = " + String.format("%.2f", A) + " B = " + String.format("%.2f", B) + " n = " + n + ".png"));
		} catch (IOException e) {
			e.printStackTrace();
		}
